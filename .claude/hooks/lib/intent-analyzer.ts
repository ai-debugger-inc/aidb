 ...
 *
 * Uses Claude API to analyze the user's prompt and assign confidence
 * scores to each skill. Model defaults to Haiku 4.5 but can be configured
 * via CLAUDE_SKILLS_MODEL env var. Falls back to keyword matching for short
 * prompts (<10 words) or if AI analysis fails. Results are cached for 1 hour.
 *
 ...
 *
 * @example
 * const result = await analyzeIntent("Fix Java adapter", skillRules);
 * // Returns: { required: ['adapter-development'], suggested: [], fromCache: false }
 */
export async function analyzeIntent(
  ...
  try {
    ...
    for (const skill of keywordHits.required) {
      if (
        !result.required.includes(skill) &&
        !result.suggested.includes(skill)
      ) {
