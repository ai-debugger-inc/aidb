...
def regenerate_gallery() -> None:
    ...
    This function should only be triggered in RTD builds as it increases the build time
    by 30-60s. Developers can still execute this function from time to time to populate
    their local gallery images with updated files.
    """
    ...
    with sync_playwright() as p:
        ...
        for ii in range(3):
            try:
                ...
                rich_print(
                    f"Browser start timed out. Trying again (attempt {ii + 2}/3)",
                )
        ...
        for item in track(gallery_items, description="Generating screenshots..."):
            ...
            for ii in range(3):
                try:
                    ...
                    rich_print(f"Page visit start timed out for: {item['link']}")
                    rich_print(f"Trying again (attempt {ii + 2}/3)")

            # copy the 404 only if the screenshot file was not manually
            # generated by a maintainer
            if not screenshot.is_file():
                rich_print(
                    f"Could not generate screenshot for {item['title']}, using 404.",
                )
                copy(image_404, screenshot)

        # Clean up the browser since we no longer need it
        browser.close()
    rich_print(f"Finished generating gallery images at: {gallery_dir}")
