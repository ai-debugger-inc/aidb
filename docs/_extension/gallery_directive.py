...
documentation, and this can be cumbersome if the gallery is generated programmatically.
This directive wraps this particular use-case in a helper- directive to generate it with
a single YAML configuration file.

It currently exists for maintainers of the pydata-sphinx-theme, but might be abstracted
into a standalone package if it proves useful.
"""
...
class GalleryGridDirective(SphinxDirective):
    ...
    def run(self) -> list[nodes.Node]:
        ...
        if self.arguments:
            ...
            path_doc = Path(path_doc_str).parent if path_doc_str else Path()
            path_data = (path_doc / path_data_rel).resolve()
            if not path_data.exists():
                logger.info("Could not find grid data at %s.", path_data)
                return [nodes.Text("No grid data found at {path_data}.")]
