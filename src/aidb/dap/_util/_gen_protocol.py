#!/usr/bin/env python3
"""DAP Protocol Dataclass Generator v2.

A clean, modular generator for creating Python dataclasses from DAP
specification JSON. Focuses on readability, maintainability, and elegant
...
class DocstringFormatter:
    ...
    def format_with_attributes(self, description: str, fields: List[FieldSpec]) -> str:
        """Format docstring with description and numpy-style attributes
        section."""
...
class DefinitionProcessor:
    ...
    def _process_fields(
        ...
        """Process definition fields into field specifications using simplified
    ...
    def _cache_body_spec(
        ...
        """Cache body specification for generating ResponseBody/EventBody
        classes."""
        properties = flattened.get("properties", {})
    ...
    def _resolve_ref_definition(self, ref_path: str) -> Dict[str, Any]:
        ...
        Args:
            ref_path: Reference path like '#/definitions/Capabilities'
    ...
    def _determine_category_like_analyzer(
        ...
        """Determine the category of a definition using analyzer's simplified
        logic."""
        # Base protocol types
...
class CodeGenerator:
    ...
    def generate_module(
        ...
        """Generate complete module code from class specifications."""

        # Combine all specs (no base class generation - it's hardcoded above the
        # marker)
        all_specs = response_body_specs + event_body_specs + class_specs

        # Sort with dependency awareness to handle inheritance properly
        sorted_specs = self._sort_by_dependencies(all_specs)

        class_codes = []
        for spec in sorted_specs:
            class_codes.append(self.generate_class(spec))
    ...
    def _sort_by_dependencies(self, specs: List[ClassSpec]) -> List[ClassSpec]:
        ...
        Classes that are used as base classes should be defined before classes
...
class ProtocolUpdater:
    """Updates the protocol.py file with generated classes."""

    GENERATION_MARKER = "# BEGIN: PROTOCOL GENERATION"

    def __init__(self, protocol_path: Path):
        self.protocol_path = protocol_path

    def update(self, generated_code: str) -> None:
        """Update protocol.py with new generated code."""
        existing_content = self._read_existing_content()
        new_content = self._merge_content(existing_content, generated_code)
        self._write_content(new_content)

    def _read_existing_content(self) -> str:
        """Read existing protocol.py content."""
        if self.protocol_path.exists():
            with open(self.protocol_path) as f:
                return f.read()
        return ""

    def _merge_content(self, existing: str, generated: str) -> str:
        """Merge existing content with generated code."""
        marker_pos = existing.find(self.GENERATION_MARKER)
        if marker_pos == -1:
            raise ValueError(
                f"Could not find '{self.GENERATION_MARKER}' in {self.protocol_path}"
            )

        # Keep everything up to and including the marker
        marker_end = existing.find("\n", marker_pos)
        if marker_end == -1:
            marker_end = len(existing)

        before_marker = existing[: marker_end + 1]

        # Add generated code
        return f"{before_marker}\n\n{generated}\n"

    def _write_content(self, content: str) -> None:
        """Write content to protocol.py file."""
...
class DAPGenerator:
    ...
    def __init__(self, spec_path: Path, protocol_path: Path):
        self.spec_path = spec_path
        self.protocol_path = protocol_path

        # Initialize components
        self.loader = SpecificationLoader(spec_path)
        self.formatter = DocstringFormatter(max_width=80)
        self.type_mapper = TypeMapper()
        self.processor = DefinitionProcessor(self.type_mapper, self.loader)
        self.generator = CodeGenerator(self.formatter)
        self.updater = ProtocolUpdater(protocol_path)
    ...
    def generate(self) -> None:
        ...
        generated_code = self.generator.generate_module(
            class_specs, response_body_specs, event_body_specs
        )

        print("Updating protocol.py...")
        self.updater.update(generated_code)

        print(
            f"Successfully generated {len(class_specs)} classes, "
            + f"{len(response_body_specs)} ResponseBody classes, "
            + f"{len(event_body_specs)} EventBody classes in {self.protocol_path}"
        )
...
def main() -> int:
    ...
    protocol_path = script_dir.parent / "protocol.py"
    ...
    try:
        generator = DAPGenerator(spec_path, protocol_path)
        generator.generate()
        return 0
