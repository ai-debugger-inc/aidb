"""Tests for JavaScript language generator."""
...
from tests.aidb_cli.generator.unit.test_utils import (
    assert_valid_javascript,
    extract_markers,
)


class TestJavaScriptGeneratorBasics:
    """Tests for basic JavaScript generator functionality."""

    def test_generator_properties(self, javascript_generator: JavaScriptGenerator):
        """Test generator properties."""
        assert javascript_generator.language_name == "javascript"
        assert javascript_generator.file_extension == ".js"
        assert javascript_generator.comment_prefix == "//"

    def test_format_marker(self, javascript_generator: JavaScriptGenerator):
        """Test marker formatting."""
        marker = javascript_generator.format_marker("var.init.x")
...
class TestVariableGeneration:
    ...
    def test_generate_variable_with_initial_value(
        ...
        code = javascript_generator.generate_variable(sample_variable_construct)

        assert "let counter = 0" in code
        assert "//:var.init.counter:" in code
        assert_valid_javascript(code)

    def test_generate_variable_increment(
        self,
        javascript_generator: JavaScriptGenerator,
    ):
        ...
        code = javascript_generator.generate_variable(construct)

        assert "count++" in code or "count += 1" in code
        assert "//:var.inc.count:" in code

    def test_generate_variable_decrement(
        self,
        javascript_generator: JavaScriptGenerator,
    ):
    ...
    def test_generate_variable_add(self, javascript_generator: JavaScriptGenerator):
    ...
    def test_generate_variable_assign(self, javascript_generator: JavaScriptGenerator):
    ...
    def test_generate_variable_no_value(
        self,
        javascript_generator: JavaScriptGenerator,
    ):
...
class TestLoopGeneration:
    ...
    def test_generate_for_loop(
        ...
        code = javascript_generator.generate_loop(sample_loop_construct)

        assert "for (let i = 0; i < 5; i++)" in code
        assert "//:flow.loop.basic:" in code
        assert_valid_javascript(code)

    def test_generate_for_loop_with_step(
        self,
        javascript_generator: JavaScriptGenerator,
    ):
        ...
        code = javascript_generator.generate_loop(construct)

        assert "i += 2" in code

    def test_generate_while_loop(self, javascript_generator: JavaScriptGenerator):
    ...
    def test_generate_foreach_loop(self, javascript_generator: JavaScriptGenerator):
        ...
        assert "for (const item of items)" in code

    def test_generate_loop_with_body(self, javascript_generator: JavaScriptGenerator):
        ...
        assert "for (let i = 0; i < 3; i++)" in code
...
class TestFunctionGeneration:
    ...
    def test_generate_function_definition(
        ...
        code = javascript_generator.generate_function(sample_function_construct)
    ...
    def test_generate_function_with_parameters(
        ...
        code = javascript_generator.generate_function(construct)

        assert "function add(a, b)" in code

    def test_generate_function_call(self, javascript_generator: JavaScriptGenerator):
    ...
    def test_generate_function_call_with_result(
        ...
        assert "const sum = add(a, b)" in code
...
class TestConditionalGeneration:
    ...
    def test_generate_conditional(
        ...
        code = javascript_generator.generate_conditional(sample_conditional_construct)
    ...
    def test_generate_conditional_without_else(
        ...
        code = javascript_generator.generate_conditional(construct)
...
class TestExceptionGeneration:
    ...
    def test_generate_exception(
        ...
        code = javascript_generator.generate_exception(sample_exception_construct)
    ...
    def test_generate_exception_with_finally(
        ...
        code = javascript_generator.generate_exception(construct)
...
class TestArrayGeneration:
    ...
    def test_generate_array_with_values(
        self,
        javascript_generator: JavaScriptGenerator,
    ):
        ...
        code = javascript_generator.generate_array(construct)

        assert "const numbers = [10, 20, 30]" in code
        assert "//:var.init.array:" in code
        assert_valid_javascript(code)

    def test_generate_array_with_range(
        self,
        javascript_generator: JavaScriptGenerator,
    ):
        ...
        assert "Array.from({length: 100}" in code
        assert "//:var.range.array:" in code
        assert_valid_javascript(code)

    def test_generate_array_with_zeros(
        self,
        javascript_generator: JavaScriptGenerator,
    ):
        ...
        assert "new Array(50).fill(0)" in code
        assert "//:var.zeros.array:" in code
        assert_valid_javascript(code)

    def test_generate_empty_array(self, javascript_generator: JavaScriptGenerator):
...
class TestPrintGeneration:
    ...
    def test_generate_print_simple(self, javascript_generator: JavaScriptGenerator):
        ...
        code = javascript_generator.generate_print(construct)

        assert 'console.log("Hello, World!")' in code
        assert "//:func.print.hello:" in code

    def test_generate_print_with_variable(
        self,
        javascript_generator: JavaScriptGenerator,
    ):
...
class TestProgramGeneration:
    ...
    def test_format_program(
        ...
        code = javascript_generator.generate_program(simple_scenario)
...
main();
"""
        result = javascript_generator.validate_syntax(code)
    ...
    def test_validate_syntax_invalid_code(
        ...
        code = "function broken() { if (true) { } "  # Unbalanced braces
...
class TestMarkerEmbedding:
    ...
    def test_marker_format_consistency(
        self,
        javascript_generator: JavaScriptGenerator,
    ):
...
class TestSyntaxErrorGeneration:
    ...
    def test_generate_syntax_error_unclosed_bracket(
        ...
        code = javascript_generator.generate_syntax_error(construct)

        assert "function brokenFunction" in code
    ...
    def test_generate_syntax_error_missing_semicolon(
        ...
        assert "let x = 5" in code
        assert "//:error.syntax.semicolon:" in code

    def test_generate_syntax_error_unclosed_brace(
        self,
        javascript_generator: JavaScriptGenerator,
    ):
        """Test generating syntax error with unclosed brace."""
        from aidb_cli.generators.core.types import SyntaxErrorConstruct

        construct = SyntaxErrorConstruct(
            error_type="unclosed_brace",
            marker="error.syntax.brace",
        )

        code = javascript_generator.generate_syntax_error(construct)

        assert "if (true)" in code
        assert "//:error.syntax.brace:" in code
    ...
    def test_generate_syntax_error_with_code_snippet(
        ...
        construct = SyntaxErrorConstruct(
            code_snippet="const x = [1, 2, 3",
            marker="error.syntax.custom",
        )

        code = javascript_generator.generate_syntax_error(construct)

        assert "const x = [1, 2, 3" in code
        assert "//:error.syntax.custom:" in code

    def test_syntax_error_has_marker(self, javascript_generator: JavaScriptGenerator):
...
class TestInfiniteLoopGeneration:
    ...
    def test_infinite_loop_has_marker(
        self,
        javascript_generator: JavaScriptGenerator,
    ):
...
class TestRecursiveFunctionGeneration:
    ...
    def test_recursive_function_definition(
        ...
        assert "function recursiveFunction(depth)" in code
        assert "console.log(`Depth: ${depth}`)" in code
    ...
    def test_recursive_with_parameter(
        ...
        assert "function recurse(n)" in code
...
class TestLargeArrayGeneration:
    ...
    def test_large_array_with_range(self, javascript_generator):
        ...
        assert "const largeNumbers = Array.from({length: 3000}, (_, i) => i);" in code
        assert "//:var.create.large_array:" in code

    def test_large_array_iteration(self, javascript_generator):
        ...
        code = javascript_generator.generate_loop(loop_construct)

        assert "for (const item of largeNumbers) {" in code
        assert "if (item % 500 === 0) {" in code
        assert "console.log(`Checkpoint: ${item}`);" in code
        assert "//:flow.loop.iterate:" in code
        assert "//:flow.if.checkpoint:" in code
        assert "//:func.print.checkpoint:" in code

    def test_large_array_with_zeros(self, javascript_generator):
        ...
        assert "const zerosArray = new Array(3000).fill(0);" in code
        assert "//:var.create.zeros:" in code

    def test_large_array_checkpoint_logic(self, javascript_generator):
        ...
        code = javascript_generator.generate_conditional(conditional_construct)
    ...
    def test_large_array_scenario_markers(self, javascript_generator):
        ...
        array_code = javascript_generator.generate_array(array_construct)
        loop_code = javascript_generator.generate_loop(loop_construct)
